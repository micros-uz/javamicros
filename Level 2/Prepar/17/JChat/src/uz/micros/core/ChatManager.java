...
public class ChatManager
    ...
    private String hostUserName = "Admin";//"Ustoz";//"先生";
    ...
    private void startConnectors() {
        try {
            ...
            for (int k = local[3]; k <= local[3]; k++){
                InetAddress ia = getAddr(local, k);
                new Thread(new Connector(ia, serverPort, this)).start();
            }
    ...
    private InetAddress getAddr(byte[] local, int k) {
